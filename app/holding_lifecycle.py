...
class HoldingRecord:
    ...
    def create(
        ...
        *,
        ...
        return cls(
            holding_id=holding_id or f"HOLD-{uuid4().hex[:16]}",
            opportunity_id=opportunity_id,
            lifecycle_opportunity_id=lifecycle_opportunity_id,
            decision_record_id=decision_record_id,
            transaction_id=transaction_id,
            ...
            asset_instrument_reference=asset_instrument_reference,
            ownership_interest_reference=ownership_interest_reference,
            acquisition_effective_date=acquisition_effective_date,
            acquisition_basis_reference=acquisition_basis_reference,
            capital_structure_references=tuple(capital_structure_references),
            provenance_references=tuple(provenance_references),
            audit_references=tuple(audit_references),
            state=(
                HoldingState.EXECUTION_EVIDENCE_PENDING
                if not execution_reference or not execution_reference.strip()
                else HoldingState.PROPOSED
            ),
    ...
    @property
    def funding_authority(self) -> bool:
        return False

    @property
    def closing_authority(self) -> bool:
        return False

    @property
    def brokerage_authority(self) -> bool:
        return False
    ...
    def mark_execution_evidence_pending(
        self, *, reason: str = "Execution evidence pending", at: Optional[datetime] = None
    ) -> "HoldingRecord":
        return self._transition(HoldingState.EXECUTION_EVIDENCE_PENDING, reason, at=at)

    def establish_from_execution_fact(
        self,
        *,
    ...
    def mark_disposition_pending(
        self, *, reason: str = "Disposition pending", at: Optional[datetime] = None
    ) -> "HoldingRecord":
    ...
    def _transition(
        ...
        *,
        ...
        if not isinstance(to_state, HoldingState):
            raise TypeError("to_state must be a HoldingState")
        ...
        timestamp = at or datetime.now(timezone.utc)
        operating = "ACTIVE" if to_state is HoldingState.HOLDING_ACTIVE else self.operating_status
        return HoldingRecord(
            **self._data_without_history(),
            state=to_state,
