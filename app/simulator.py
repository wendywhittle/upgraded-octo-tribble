...
def _annual_debt_service(loan,rate,amortization_years,interest_only=False):
    if loan<=0: return 0.0,"none"
    if rate is None or rate<0: return 0.0,"unknown"
...
def _remaining_balance(loan,rate,amortization_years,years,interest_only=False):
    if loan<=0 or interest_only or amortization_years is None or amortization_years<=0: return max(0.0,loan)
    if rate is None: return max(0.0,loan)
    if rate==0: return max(0.0,loan*(1-years/amortization_years))
    r,n,k=rate/12,amortization_years*12,min(amortization_years*12,years*12); pmt=loan*r/(1-(1+r)**-n)
...
def _cre_path(purchase_price,noi,hold_period,rng,growth_mu,growth_vol,occupancy,exit_cap_rate,debt,debt_service,amortization_years,interest_rate,capital_expenditures,gross_rent,operating_expenses,expense_growth,vacancy,shock,selling_cost_rate,other_income=0.0,interest_only=False):
    ...
    for year in range(1,hold_period+1):
        ...
        if gross_rent is not None and operating_expenses is not None:
            rent=gross_rent*(1+growth)**year; occ=max(0,min(1,occupancy+rng.gauss(0,growth_vol))); vacancy_rate=vacancy if vacancy is not None else 1-occ
            current_noi=rent*max(0,1-vacancy_rate)+other_income-operating_expenses*(1+expense_growth)**year
        else:
            current_noi=current_noi*(1+growth)
    ...
    exit_value=current_noi/exit_cap_rate; net_sale=exit_value*(1-selling_cost_rate)-_remaining_balance(debt,interest_rate,amortization_years,hold_period,interest_only); total=sum(cash_flows)+net_sale
...
def run_cre_monte_carlo(purchase_price,noi,hold_period=5,paths=5000,seed=42,occupancy=None,rent_growth=None,expense_growth=None,exit_cap_rate=None,interest_rate=None,loan_to_value=None,capital_expenditures=None,assumptions=None,gross_rent=None,operating_expenses=None,vacancy=None,amortization_years=None,selling_cost_rate=None,interest_only=False,closing_costs=None,other_income=None,scenario_overrides:Mapping[str,Mapping[str,float]]|None=None):
    ...
    debt=price*ltv
    if debt>0 and interest_rate is None: return {"engine":"AletheiaTelos Independent CRE Monte Carlo Risk Engine v2","independent_of_agents":True,"status":"INSUFFICIENT EVIDENCE","missing_inputs":("interest_rate",),"scenarios":[],"assumptions":assumptions or []}
    if debt>0 and not interest_only and (amortization_years is None or amortization_years<=0): return {"engine":"AletheiaTelos Independent CRE Monte Carlo Risk Engine v2","independent_of_agents":True,"status":"INSUFFICIENT EVIDENCE","missing_inputs":("amortization_years",),"scenarios":[],"assumptions":assumptions or []}
    rate=float(interest_rate or 0.0); ds,method=_annual_debt_service(debt,rate,amortization_years,interest_only); equity_initial=price+float(closing_costs or 0)-debt; capex=float(capital_expenditures or 0); sale_cost=float(selling_cost_rate or 0); scenario_overrides=scenario_overrides or {}
...
def cre_sensitivity(purchase_price,noi,hold_period=5,occupancy=None,rent_growth=None,interest_rate=None,loan_to_value=None,exit_cap_rate=None,expense_growth=None,operating_expenses=None,capital_expenditures=None,paths=1000,seed=42):
    ...
    base_cap=exit_cap_rate or noi/purchase_price; base_kwargs=dict(occupancy=occupancy,rent_growth=rent_growth,interest_rate=interest_rate,loan_to_value=loan_to_value,exit_cap_rate=base_cap,expense_growth=expense_growth,operating_expenses=operating_expenses,capital_expenditures=capital_expenditures); base=run_cre_monte_carlo(purchase_price,noi,hold_period,paths,seed,**base_kwargs)
