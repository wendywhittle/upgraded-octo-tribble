...
def _irr(cash_flows):
    if not cash_flows or not (any(v > 0 for v in cash_flows) and any(v < 0 for v in cash_flows)):
        return None
    def npv(rate):
        return sum(v / (1 + rate) ** i for i, v in enumerate(cash_flows))
    low, high = -.9999, 10.0
    low_value = npv(low)
...
def _cre_path(purchase_price, noi, hold_period, rng, growth_mu, growth_vol, occupancy, exit_cap_rate, debt, debt_service, amortization_years, interest_rate, capital_expenditures, gross_rent, operating_expenses, expense_growth, vacancy, shock, selling_cost_rate, other_income=0.0, interest_only=False):
    ...
    for year in range(1, hold_period + 1):
        ...
        if gross_rent is not None and operating_expenses is not None:
            ...
            occ = max(0, min(1, occupancy + rng.gauss(0, growth_vol)))
...
def run_cre_monte_carlo(purchase_price, noi, hold_period=None, paths=5000, seed=42, occupancy=None, rent_growth=None, expense_growth=None, exit_cap_rate=None, interest_rate=None, loan_to_value=None, capital_expenditures=None, assumptions=None, gross_rent=None, operating_expenses=None, vacancy=None, amortization_years=None, selling_cost_rate=None, interest_only=False, closing_costs=None, other_income=None, scenario_overrides: Mapping[str, Mapping[str, float]] | None = None):
    required = {"purchase_price": purchase_price, "noi": noi, "hold_period": hold_period, "exit_cap_rate": exit_cap_rate}
    missing = tuple(k for k, v in required.items() if v is None or (isinstance(v, (int, float)) and v <= 0))
    if missing:
    ...
    if missing:
        return {"engine": "AletheiaTelos Independent CRE Monte Carlo Risk Engine v2", "independent_of_agents": True, "status": "INSUFFICIENT EVIDENCE", "missing_inputs": tuple(dict.fromkeys(missing)), "scenarios": [], "assumptions": assumptions or []}
    price, base_noi = float(purchase_price), float(noi)
    occ = float(occupancy)
    growth = float(rent_growth)
    exp_growth = float(expense_growth)
    exit_cap = float(exit_cap_rate)
    ltv = float(loan_to_value)
    if not 0 <= ltv < 1:
        raise ValueError("loan_to_value must be between 0 and 1")
    debt = price * ltv
    if debt > 0 and interest_rate is None:
    ...
    for name, default in defaults.items():
        ...
        g = growth + spec["rent_growth_delta"]
        o = max(0, min(1, occ + spec["occupancy_delta"]))
        ec = max(.0001, exit_cap + spec["exit_cap_delta"])
        rng = random.Random(seed + sum(map(ord, name)))
        outcomes, exits, dscrs, mins, cocs, mults, irrs, dds = [], [], [], [], [], [], [], []
        for _ in range(paths):
            p = _cre_path(price, base_noi, hold_period, rng, g, spec["growth_vol"], o, ec, debt, ds, amortization_years, rate, capex, gross_rent, operating_expenses, exp_growth, vacancy, spec["shock"], sale_cost, float(other_income or 0), interest_only)
            outcomes.append(p["equity_outcome"])
            exits.append(p["exit_value"])
            dds.append(_drawdown(p["equity_values"]))
...
def cre_sensitivity(purchase_price, noi, hold_period=None, occupancy=None, rent_growth=None, interest_rate=None, loan_to_value=None, exit_cap_rate=None, expense_growth=None, operating_expenses=None, capital_expenditures=None, paths=1000, seed=42):
    ...
    for variable, values in cases.items():
        ...
        for value, label in values:
            ...
            sim = run_cre_monte_carlo(pp, noi, hold_period, paths, seed, **kwargs)
            row = next((s for s in sim.get("scenarios", []) if s["scenario"] == "BASE"), None)
            rows.append({"value": label, "base_scenario": row})
