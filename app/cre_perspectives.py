...
def _assessment(ctx: CREOpportunityContext, perspective: str, thesis: str, risks: tuple[str, ...], questions: tuple[str, ...], invalidations: tuple[str, ...], recommendation: str = "WATCH", financial_result: Any | None = None, claims: tuple[str, ...] = ()) -> PerspectiveAssessment:
    ...
    return PerspectiveAssessment(
        perspective_id=perspective, thesis=thesis, risks=risks, unanswered_questions=questions,
        invalidation_conditions=invalidations, recommendation=recommendation,
        confidence=None if ctx.missing_required() else 0.5, time_horizon="investment horizon",
        economic_claims=claims, audit_metadata=("deterministic_structured_lens", "shared_economic_model", "human_authority_required"), **data,
    )
...
def _underwriter(ctx, model):
    ...
    if missing:
        return _assessment(ctx, "underwriter", "Underwriting cannot be responsibly completed until required inputs are verified.", ("incomplete economics",), tuple(missing), ("Material NOI or financing input changes",), "INSUFFICIENT EVIDENCE", model)
    cap, dscr = model.going_in_cap_rate, model.dscr
    claims = tuple(x for x in (f"Going-in cap rate is {cap:.2%}." if cap is not None else None, f"Base DSCR is {dscr:.2f}x." if dscr is not None else None) if x)
...
def _investor(ctx, model):
    coc, irr_value, net_sale = getattr(model, "cash_on_cash", None), getattr(model, "irr", None), getattr(model, "net_sale_proceeds", None)
    claims = tuple(x for x in (f"First-year cash-on-cash is {coc:.2%}." if coc is not None else None, f"Modeled IRR is {irr_value:.2%}." if irr_value is not None else None, f"Net sale proceeds are {net_sale:,.0f}." if net_sale is not None else None) if x)
...
def _researcher(ctx, model):
    ...
    unknowns = tuple(f"Verify provenance for {name}." for name in ("rent_growth", "exit_cap_rate") if getattr(ctx, name, None) is not None)
...
def _risk(ctx, model):
    ...
    return _assessment(ctx, "risk", "Capital preservation requires explicit downside, leverage, liquidity, concentration and tail-risk analysis before deployment.", ("leverage", "liquidity", "tail risk"), ("What is the plausible permanent loss path?",), ("Downside cannot be contained within the configured mandate",), financial_result=model, claims=claims)
