...
@app.get("/")
def root(): return {"system": "AletheiaTelos", "status": "operational", "version": "1.8.0", "live_market_data": live_market_enabled()}
...
def simulate(request: SimulationRequest):
    # The registry receives only evidence that has crossed the integrity boundary.
    # The current demo route has no external evidence input, so the contract
    # correctly returns NO_DATA rather than manufacturing investment evidence.
    ...
    return {"system": "AletheiaTelos", "version": "1.8.0", "question": request.question, "timestamp": timestamp(), "agents": agents,
            ...
            "audit": {"reproducible": True, "simulation_seed": request.seed, "point_in_time_evidence_required": True, "evidence_gate": "active", "memory_recorded": True, "agent_registry": "active"}}
