...
app = FastAPI(
    title="AletheiaTelos",
    version="1.3.0",
    description="Research and decision intelligence system; not an autonomous trading system.",
)
...
def make_agent(agent_id: str, strategy: str, direction: str, confidence: float, horizon: str,
               thesis: str, contradictory: str) -> Dict[str, Any]:
    return {
        "agent_id": agent_id,
        "strategy": strategy,
        "direction": direction,
        ...
        "evidence": [{
            ...
            "provenance": {"type": "synthetic_demo", "point_in_time": True},
        }],
        "contradictory_evidence": [{
            ...
            "observed_at": timestamp(),
            "retrieved_at": timestamp(),
            "provenance": {"type": "synthetic_demo", "independent": False},
        }],
        "invalidation_conditions": ["Material evidence contradicts the core thesis."],
        "data_timestamp": timestamp(),
        "model_version": "simulation-1.1",
...
def generate_agents(question: str) -> List[Dict[str, Any]]:
    return [
        ...
        make_agent("meta_intelligence", "Meta-Intelligence", "NEUTRAL", 0.67, "medium",
                   "The disagreement between bullish and defensive perspectives is decision-relevant.",
                   "Some disagreement may arise from different assumptions rather than true conflict."),
        make_agent("governance", "CHARTER", "NEUTRAL", 0.95, "all",
...
def detect_conflicts(agents: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    for i, first in enumerate(agents):
        for second in agents[i + 1:]:
            if first.get("direction") in {"NEUTRAL", "NO_DATA"} or second.get("direction") in {"NEUTRAL", "NO_DATA"}:
                continue
            if first.get("direction") == second.get("direction"):
                continue
            if first.get("horizon") == second.get("horizon"):
                conflicts.append({"agent_a": first["agent_id"], "agent_b": second["agent_id"],
                                  "direction_a": first["direction"], "direction_b": second["direction"],
                                  "horizon": first["horizon"], "type": "same_horizon_conflict"})
            else:
                horizon_divergences.append({"agent_a": first["agent_id"], "agent_b": second["agent_id"],
                                            "direction_a": first["direction"], "direction_b": second["direction"],
                                            "horizon_a": first["horizon"], "horizon_b": second["horizon"],
...
@app.get("/")
def root():
    return {"system": "AletheiaTelos", "status": "operational", "version": "1.3.0"}
...
def simulate(request: SimulationRequest):
    ...
    simulation = run_monte_carlo(request.initial_value, request.horizon_steps, request.paths, request.seed,
    ...
    return {
        ...
        "audit": {"reproducible": True, "simulation_seed": request.seed, "point_in_time_evidence_required": True},
    }
