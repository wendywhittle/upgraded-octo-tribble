...
class CalculationResult(BaseModel):
    """Immutable calculation output with explicit inputs and provenance."""
...
def effective_gross_income(gross_potential_income: float, vacancy_credit_loss: float, *, input_refs: tuple[str, ...] = ()) -> CalculationResult:
    return _result("effective_gross_income", gross_potential_income - vacancy_credit_loss, {"gross_potential_income": gross_potential_income, "vacancy_credit_loss": vacancy_credit_loss}, input_refs)


def net_operating_income(effective_gross_income_value: float, operating_expenses: float, *, input_refs: tuple[str, ...] = ()) -> CalculationResult:
    return _result("net_operating_income", effective_gross_income_value - operating_expenses, {"effective_gross_income": effective_gross_income_value, "operating_expenses": operating_expenses}, input_refs)


def cap_rate(noi: float, property_value: float, *, input_refs: tuple[str, ...] = ()) -> CalculationResult:
    if property_value == 0:
...
def implied_value(noi: float, cap_rate_value: float, *, input_refs: tuple[str, ...] = ()) -> CalculationResult:
    if cap_rate_value == 0:
...
def loan_amount(property_value: float, ltv_value: float, *, input_refs: tuple[str, ...] = ()) -> CalculationResult:
...
def annual_debt_service(principal: float, annual_interest_rate: float, amortization_years: int, *, input_refs: tuple[str, ...] = ()) -> CalculationResult:
    if principal < 0:
    ...
    monthly_payment = principal / periods if monthly_rate == 0 else principal * monthly_rate / (1 - (1 + monthly_rate) ** (-periods))
    return _result("annual_debt_service", monthly_payment * 12, {"principal": principal, "annual_interest_rate": annual_interest_rate, "amortization_years": float(amortization_years)}, input_refs)


def dscr(noi: float, annual_debt_service_value: float, *, input_refs: tuple[str, ...] = ()) -> CalculationResult:
...
def ltv(loan_value: float, property_value: float, *, input_refs: tuple[str, ...] = ()) -> CalculationResult:
    if property_value == 0:
...
def cash_flow_after_debt_service(noi: float, annual_debt_service_value: float, *, input_refs: tuple[str, ...] = ()) -> CalculationResult:
    return _result("cash_flow_after_debt_service", noi - annual_debt_service_value, {"noi": noi, "annual_debt_service": annual_debt_service_value}, input_refs)


def cash_on_cash(annual_cash_flow: float, equity_invested: float, *, input_refs: tuple[str, ...] = ()) -> CalculationResult:
