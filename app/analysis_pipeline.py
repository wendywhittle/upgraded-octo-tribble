    ...
    for i, first in enumerate(agents):
        for second in agents[i + 1:]:
            ...
            if first.get("horizon") == second.get("horizon"):
                conflicts.append({
                    "agent_a": first["agent_id"], "agent_b": second["agent_id"],
                    "direction_a": first_direction, "direction_b": second_direction,
                    "horizon": first.get("horizon"), "type": "same_horizon_conflict",
                })
            else:
                horizon_divergences.append({
                    "agent_a": first["agent_id"], "agent_b": second["agent_id"],
                    "direction_a": first_direction, "direction_b": second_direction,
                    "horizon_a": first.get("horizon"), "horizon_b": second.get("horizon"),
...
def run_analysis(question: str, evidence: Iterable[Dict[str, Any]], initial_value: float = 100.0, horizon_steps: int = 60, paths: int = 5000, seed: int = 42, now=None, max_age_seconds: float = 24 * 60 * 60) -> Dict[str, Any]:
    ...
    return {
        "system": "AletheiaTelos", "question": question,
        ...
        "agents": agents, "conflicts": conflict_data["conflicts"], "horizon_divergences": conflict_data["horizon_divergences"],
        "simulation": simulation, "skeptic": skeptic, "synthesis": synthesis, "observer": observer, "governance": governance,
        "audit": {"pipeline": "evidence->agents->conflict->independent_risk->skeptic->synthesis->governance->observer->memory", "simulation_independent_of_agents": True, "simulation_seed": seed, "memory_recorded": True, "research_only": True, "human_decision_required": True},
    }
