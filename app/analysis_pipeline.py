...
def detect_conflicts(agents: list[Dict[str, Any]]) -> Dict[str, list[Dict[str, Any]]]:
    ...
    for record in records:
        ...
        if record["conflict_type"] == "horizon": horizon_divergences.append(item)
...
def run_analysis(question: str, evidence: Iterable[Dict[str, Any]], initial_value: float = 100.0, horizon_steps: int = 60, paths: int = 5000, seed: int = 42, now=None, max_age_seconds: float = 24 * 60 * 60, provider: ModelProvider | None = None, research_context: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    agent_stage = run_evidence_fed_agents(question, evidence, now=now, max_age_seconds=max_age_seconds, provider=provider, learning_context=learning_context, research_context=research_context)
    ...
    decision_gate = build_decision_gate({"count": agent_stage["evidence_count"], "usable_count": agent_stage["usable_evidence_count"], "validation": agent_stage["validation"]}, conflict_data, simulation, skeptic, synthesis, governance, contrarian_status="present" if any(a.get("agent_id") == "contrarian" for a in agents) else "not_available")
