...
def _decision_dict(decision):
    if decision is None:
        return None
    return {"state": decision.state.value, "rationale": decision.rationale, "criteria_satisfied": tuple(decision.criteria_satisfied), "criteria_failed": tuple(decision.criteria_failed), "unresolved_questions": tuple(decision.unresolved_questions), "evidence_ids": tuple(decision.evidence_ids), "human_decision_required": decision.human_decision_required, "autonomous_execution": decision.autonomous_execution}
...
def run_analysis(question: str,evidence: Iterable[Dict[str,Any]],initial_value: float=100.0,horizon_steps: int=60,paths: int=5000,seed: int=42,now=None,max_age_seconds: float=24*60*60,provider: ModelProvider|None=None,cre_assessments: Iterable[Dict[str,Any]]=(),cre_context: CREOpportunityContext|None=None,cre_criteria: CREDecisionCriteria|None=None):
    ...
    if cre_context is not None:
        ...
        cre_decision=decision; cre_for_view=assessments; cre_result={"context":cre_context,"underwriting":underwriting,"financial_model":financial_model,"perspectives":assessments,"conflicts":cre_conflicts,"simulation":cre_simulation,"adversarial_review":adversarial,"decision":decision,"decision_boundary":"human authority required"}
    record["decision"]=_decision_dict(cre_decision); record["meta_intelligence"]=meta; append_record(record)
