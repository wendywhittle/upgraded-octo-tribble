...
CREEvidenceCategory = Literal["listing", "broker_statement", "property_record", "operating_statement", "rent_roll", "lease", "tenant", "market", "sales_comparable", "debt_term", "environmental", "physical_due_diligence", "other"]
...
def validate_cre_evidence(evidence: CREEvidence, *, now: datetime | None = None, max_age_seconds: float = 24 * 60 * 60) -> dict[str, Any]:
    """Reuse the generic integrity gate through a thin validation view."""
    validation_view = {
        ...
        "provenance": {"type": evidence.provenance.source_type or "external_source", "point_in_time": evidence.provenance.point_in_time, **evidence.provenance.metadata},
    }
    return validate_evidence(validation_view, now=now, max_age_seconds=max_age_seconds)

def build_cre_evidence(*, evidence_id: str, opportunity_id: str, category: CREEvidenceCategory, field_name: str, observed_value: str | int | float | bool | None = None, source_content: str | None = None, observation_type: str = "source_observation", status: CREEvidenceStatus = "sourced", provenance: CREEvidenceProvenance, uncertainty: list[str] | None = None, freshness_seconds: float | None = None, validation_status: Literal["unvalidated", "verified", "blocked"] = "unvalidated", decision_usable: bool = False, corroboration_refs: list[str] | None = None, dependent_evidence_refs: list[str] | None = None, conflict_refs: list[str] | None = None, contradictory_evidence_refs: list[str] | None = None, supersedes_evidence_id: str | None = None, subject_ref: str | None = None, created_at: datetime | None = None) -> CREEvidence:
    return CREEvidence(
        evidence_id=evidence_id, opportunity_id=opportunity_id, category=category, subject_ref=subject_ref,
        field_name=field_name, observed_value=observed_value, source_content=source_content,
        observation_type=observation_type, status=status, provenance=provenance,
        uncertainty=list(dict.fromkeys(uncertainty or [])), freshness_seconds=freshness_seconds,
        validation_status=validation_status, decision_usable=decision_usable,
        ...
        supersedes_evidence_id=supersedes_evidence_id, created_at=created_at or utc_now(),
    )
