...
def model_inputs(**overrides):
    values = dict(
        purchase_price=10_000_000, noi=700_000, occupancy=0.95, rent_growth=0.03,
        expense_growth=0.02, loan_to_value=0.65, interest_rate=0.06,
        amortization_years=25, hold_period_years=5, exit_cap_rate=0.07,
        selling_cost_rate=0.02, capital_expenditures=25_000,
...
def test_amortizing_balance_reduces_and_interest_only_does_not():
    amortized = underwrite_financial_model(model_inputs())
    io = underwrite_financial_model(model_inputs(interest_only=True))
...
def test_break_even_and_configured_thresholds_are_explicit():
    inputs = model_inputs(assumptions=(Assumption("target_cap_rate", .075, "ratio"), Assumption("minimum_dscr", 1.25, "x")))
    result = underwrite_financial_model(inputs)
    assert result.break_even_occupancy is None  # no gross-rent/expense ledger was supplied
...
def test_operating_ledger_derives_noi_when_supplied_noi_is_still_the_underwriting_anchor():
...
def test_explicit_criteria_can_produce_no_deal_without_authority():
    ...
    decision = evaluate_financial_decision(result, CREDecisionCriteria(min_dscr=1.20, max_ltv=0.75, min_irr=0.12), supplied_ltv=0.80)
...
def test_financing_unknown_prevents_simulation_from_inventing_rate():
    ...
    assert result["status"] == "INSUFFICIENT EVIDENCE"
    assert "interest_rate" in result["missing_inputs"]
