...
from app.workflow import InvestmentCase, WorkflowService, WorkflowState
...
def test_scenario_and_simulation_interpretation_consumes_existing_outputs():
    ...
    result = synthesize_investment_case(
        case_identity="case-1", scenarios=scenarios, simulations=simulations
    )
        ...
        disagreements=[
            {
                ...
                "perspectives": ["researcher", "investor"],
                "positions": ["stable", "declining"],
                "unresolved_questions": ["Which rent estimate is supported?"],
            }
        ],
    )
...
def test_contradictory_evidence_is_not_silently_resolved():
    result = synthesize_investment_case(
        case_identity="case-1",
        proforma=calculate_proforma(base_input()),
        evidence=[
...
def test_agent_probability_and_irr_cannot_become_authoritative():
    for key, value in [("probability_loss", 0.12), ("irr", 0.18), ("unlevered_irr", 0.18)]:
        with pytest.raises(SynthesisValidationError):
            synthesize_investment_case(
                case_identity="case-1",
                evidence=[{"evidence_id": "e1", "claim": "x"}],
                agent_perspectives=[{"agent_id": "quant", key: value}],
            )
...
def test_investment_case_integration_is_copy_only():
    case = InvestmentCase(
        deal_id="case-1",
        workflow_state=WorkflowState.AWAITING_INVESTMENT_APPROVAL,
        human_authorizations=["auth-1"],
    )
    result = synthesize_investment_case(
        case_identity="case-1", proforma=calculate_proforma(base_input())
    )
...
def test_no_go_recommendation_remains_recommendation_only():
    ...
    finding = ContrarianFinding(
        category="ASSUMPTION", severity="CRITICAL", description="NO_GO_RECOMMENDATION",
        rationale="Insufficient support", provenance="RECOMMENDATION", output_type="RECOMMENDATION",
    )
...
def test_identical_inputs_produce_identical_synthesis():
    ...
    first = synthesize_investment_case(
    ...
    second = synthesize_investment_case(
        case_identity="case-1", proforma=proforma, scenarios=scenarios,
        simulations=simulations, contrarian=contrarian, evidence=evidence,
    )
...
def test_calculation_outputs_remain_calculation_provenance():
    ...
    result = synthesize_investment_case(
        case_identity="case-1", proforma=proforma, simulations=simulations
    )
    assert result.output_type == "INTERPRETATION"
    assert "CALCULATION" in result.provenance["layers"] or result.provenance["layers"]


def test_synthesis_does_not_change_workflow_or_create_authorization():
    service = WorkflowService()
    deal = service.create_deal(opportunity={"name": "test"})
    case = InvestmentCase(deal_id=deal.deal_id, workflow_state=deal.state)
    result = synthesize_investment_case(
        case_identity=deal.deal_id, proforma=calculate_proforma(base_input())
    )
    updated = apply_synthesis_to_case(case, result)
...
def test_unresolved_disagreement_prevents_false_certainty():
    result = synthesize_investment_case(
        ...
        disagreements=[
            {
                "topic": "Exit assumptions",
                "status": "UNRESOLVED",
                "perspectives": ["investor", "contrarian"],
                "positions": ["supportive", "skeptical"],
                "unresolved_questions": ["What exit assumption is independently supported?"],
            }
        ],
    )
    assert result.thesis_status == "CONDITIONAL"
