...
from app.investment_synthesis import SynthesisValidationError, apply_synthesis_to_case, synthesize_investment_case
...
def base_input():
    return ProFormaInput(asset_id="asset-1", acquisition_price=10_000_000, acquisition_costs=250_000, projection_years=5, annual_rent=900_000, initial_occupancy=0.95, vacancy_rate=0, operating_expenses=250_000, entry_cap_rate=0.065, exit_cap_rate=0.06, rent_growth=0.04)
...
def analytical_fixture(seed=42):
    ...
    simulations = [run_cre_simulation(inputs, d, config=CRESimulationConfig(paths=100, horizon_steps=5, seed=seed)) for d in definitions.scenarios]
    evidence = [{"evidence_id": "e1", "claim": "rent roll supports current rent", "source": "lease"}]
    contrarian = review_contrarian(case_identity="case-1", proforma=proforma, scenarios=scenarios, simulations=simulations, evidence=evidence)
...
def test_synthesizes_complete_investment_case():
    ...
    result = synthesize_investment_case(case_identity="case-1", proforma=proforma, scenarios=scenarios, simulations=simulations, contrarian=contrarian, evidence=evidence, agent_perspectives=[{"agent_id": "investor", "assumptions": ["Stable tenancy"]}])
    ...
    assert result.provenance["layers"][-1] == "META_INTELLIGENCE"
...
def test_scenario_and_simulation_interpretation_consumes_existing_outputs():
    ...
    combined = " ".join(result.thesis.scenario_resilience + result.thesis.simulation_observations)
...
def test_disagreement_is_preserved():
    result = synthesize_investment_case(case_identity="case-1", evidence=[{"evidence_id": "e1", "claim": "market rent", "source": "A"}], disagreements=[{"topic": "Market rent", "status": "MATERIAL_DISAGREEMENT", "perspectives": ["researcher", "investor"], "positions": ["stable", "declining"], "unresolved_questions": ["Which rent estimate is supported?"]}])
...
def test_contradictory_evidence_is_not_silently_resolved():
    result = synthesize_investment_case(case_identity="case-1", proforma=calculate_proforma(base_input()), evidence=[{"evidence_id": "e1", "claim": "tenant retention", "source": "lease", "contradictory": True}, {"evidence_id": "e2", "claim": "tenant retention", "source": "broker", "contradictory": True}])
...
def test_upstream_results_are_not_mutated_by_synthesis():
    _, proforma, scenarios, simulations, evidence, contrarian = analytical_fixture()
    snapshots = {"proforma": copy.deepcopy(proforma.model_dump()), "scenarios": copy.deepcopy([x.model_dump() for x in scenarios]), "simulations": copy.deepcopy([x.model_dump() for x in simulations]), "contrarian": copy.deepcopy(contrarian.model_dump())}
...
def test_unresolved_disagreement_prevents_false_certainty():
    result = synthesize_investment_case(case_identity="case-1", proforma=calculate_proforma(base_input()), disagreements=[{"topic": "Exit assumptions", "status": "UNRESOLVED", "perspectives": ["investor", "contrarian"], "positions": ["supportive", "skeptical"], "unresolved_questions": ["What exit assumption is independently supported?"]}])
    assert result.thesis_status == "CONDITIONAL"
