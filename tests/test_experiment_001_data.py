from app.experiment_001_data import HistoricalPoint, build_experiment_observations
...
def test_builder_uses_only_prior_data_for_features_and_future_data_for_label():
    ...
    # Observation index 5 compares only index 5 with index 0 for 5-day features.
    ...
    # The label uses only the next five SPX observations.
...
def test_point_in_time_violation_is_rejected():
    ...
    points[7] = HistoricalPoint(
        observed_at=points[7].observed_at,
        spx_close=points[7].spx_close,
        vix_level=points[7].vix_level,
        skew_level=points[7].skew_level,
        available_at="2025-01-08T22:00:00+00:00",
        source_id="fixture",
        content_hash="hash-7",
    )
    try:
...
def test_missing_provenance_is_rejected():
    ...
    points[3] = HistoricalPoint(
        observed_at=points[3].observed_at,
        spx_close=points[3].spx_close,
        vix_level=points[3].vix_level,
        skew_level=points[3].skew_level,
        available_at=points[3].available_at,
        source_id="",
        content_hash="hash-3",
    )
    try:
...
def test_future_label_is_not_included_as_a_feature():
    points_a = _points()
    points_b = _points()
    # Alter only a future price after the first feature timestamp.
    points_b[8] = HistoricalPoint(
        observed_at=points_b[8].observed_at,
        spx_close=4000,
        vix_level=points_b[8].vix_level,
        skew_level=points_b[8].skew_level,
        available_at=points_b[8].available_at,
        source_id=points_b[8].source_id,
