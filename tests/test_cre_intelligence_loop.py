...
def test_missing_inputs_are_preserved_and_not_fabricated():
    ...
    assert "noi" in result["missing_inputs"]


def test_cre_conflict_projection_preserves_disagreement_dimensions():
    assessments = list(assess_cre_opportunity(make_context()))
    assessments[0] = assessments[0].__class__(**{**assessments[0].__dict__, "assumptions": ("normalized NOI", "exit cap verified")})
    assessments[1] = assessments[1].__class__(**{**assessments[1].__dict__, "assumptions": ("aggressive rent growth", "exit cap verified")})
...
def test_governance_remains_human_only():
    simulation = run_cre_monte_carlo(10_000_000, 700_000, paths=100, seed=1, exit_cap_rate=0.07)
