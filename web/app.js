...
function renderConflicts(conflicts=[],horizon=[]){const asItems=value=>Array.isArray(value)?value:(value&&typeof value==="object"?[value]:[]);const items=[...asItems(conflicts),...asItems(horizon)];const safe=v=>{if(Array.isArray(v))return v.join(" • ");if(v==null||v==="")return "";if(typeof v==="object"){try{return JSON.stringify(v);}catch{return "";}}const text=String(v);return text==="[object Object]"?"Conflict data unavailable.":text;};$("conflict-count").textContent=`${items.length} ${items.length===1?"ITEM":"ITEMS"}`;$("conflicts").innerHTML=items.length?items.map(c=>{const item=c&&typeof c==="object"?c:{};const r=item.conflict_record&&typeof item.conflict_record==="object"?item.conflict_record:item;const question=Array.isArray(r.unresolved_questions)?r.unresolved_questions.join(" • "):safe(r.unresolved_questions||item.description)||"Unresolved question not supplied.";return `<div class="conflict-item"><strong>${esc(safe(r.conflict_type||item.type)||"CONFLICT")}</strong><span>${esc(safe(r.severity||item.severity)||"UNSPECIFIED")}</span><div class="question">${esc(question)}</div></div>`}).join(""):"NO CONFLICT DATA / NO DISAGREEMENT RETURNED";}
function renderRisk(sim={}){const scenarios=Array.isArray(sim.scenarios)?Object.fromEntries(sim.scenarios.map(x=>[x.scenario,x])):(sim.scenarios||{});const names=["base","bull","bear","adversarial"];$("risk-grid").innerHTML=names.map(n=>{const x=scenarios[n];if(!x)return `<div class="risk-card"><span>${n.toUpperCase()}</span><strong>NOT AVAILABLE</strong><em>Backend metric not returned</em></div>`;return `<div class="risk-card"><span>${n.toUpperCase()}</span><strong>${esc(x.mean_terminal??"NOT AVAILABLE")}</strong><em>loss ${x.probability_loss==null?"NOT AVAILABLE":esc(x.probability_loss)} • drawdown ${x.max_drawdown_mean==null?"NOT AVAILABLE":esc(x.max_drawdown_mean)}</em></div>`;}).join("");$("risk-note").textContent=sim.independent_of_agents===false?"INDEPENDENCE FLAG FAILED":"Independent simulation output. Agent conclusions do not determine these results.";}
function renderMemory(records=[]){const r=records.at(-1);if(!r)return;$("memory-belief").textContent=r.question||"NOT AVAILABLE";const evidence=r.evidence||{};$("memory-why").textContent=evidence.count==null?"NOT AVAILABLE":`${evidence.count} evidence item(s)`;$("memory-decision").textContent=r.synthesis?.verdict||"NOT AVAILABLE";$("memory-outcome").textContent=r.outcome?.status||"PENDING";$("memory-lesson").textContent=r.lesson||"AWAITING OUTCOME";}
