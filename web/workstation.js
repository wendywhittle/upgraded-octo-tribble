...
(() => {
  ...
  const navMap = {
    dashboard:'opportunity', analyses:'opportunity', opportunities:'opportunity', evidence:'evidence', perspectives:'perspectives', simulation:'risk',
    'investment cases':'case', 'decision gate':'gate', observer:'observer', learning:'learning'
  };
  ...
  function activate(name, detail) {
    ...
    q('.nav-item').forEach(item => {
      ...
      const mapped = navMap[labelText];
      if (mapped) item.classList.toggle('active', mapped === key);
    });
  ...
  function wireNavigation() {
    q('.nav-item[href^="#"]').forEach(item => {
      ...
      item.addEventListener('click', event => {
        ...
        const mapped = navMap[text(item).replace(/\s+NOT EXPOSED$/i,'').trim().toLowerCase()];
        if (mapped) activate(mapped, `Workspace: ${text(item)}`);
        else setActiveTarget($(id));
      });
    });
  }
  function wireStages() {
  ...
  function addStyles() {
    if($('workstation-styles'))return; const s=document.createElement('style');s.id='workstation-styles';s.textContent=`
  ...
  function observeDynamicContent(){['pipeline-stages','recent-analyses','perspectives','risk-grid','conflicts'].map($).filter(Boolean).forEach(root=>new MutationObserver(()=>{exposeLatestAnalysis();wireNavigation();wireInspectors()}).observe(root,{childList:true,subtree:true}));}
  function init(){addStyles();ensureContextBar();wireNavigation();wireStages();wireRecent();wireInspectors();wireActions();wireSearch();exposeLatestAnalysis();activate('opportunity','Current analytical workspace');announce('AletheiaTelos institutional intelligence workstation ready');}
