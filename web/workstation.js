...
(() => {
  ...
  const stageMap = {
    ...
    case:['INVESTMENT CASE',null,'case'],
    gate:['DECISION GATE',null,'gate'],
  ...
  const navMap = {
    dashboard:'opportunity', analyses:'opportunity', opportunities:'opportunity', evidence:'evidence', underwriting:'underwriting',
    perspectives:'perspectives', conflict:'conflict', simulation:'risk', 'independent risk':'risk', contrarian:'contrarian',
    'investment cases':'case', 'investment case':'case', 'decision gate':'gate', 'human authority':'human', 'decision record':'decision record',
    outcome:'outcome', observer:'observer', attribution:'attribution', disposition:'disposition', learning:'learning',
    'capital engine':'opportunity', 'asset engine':'opportunity', 'epistemic memory':'memory', reports:'memory', 'system status':'opportunity'
  };

  const maturity = {
    'DECISION RECORD':'NOT YET OPERATIONAL',
    'EPISTEMIC MEMORY':'DEVELOPING',
    'OUTCOME':'DEVELOPING',
    'ATTRIBUTION':'NOT YET OPERATIONAL',
    'DISPOSITION':'NOT YET OPERATIONAL',
    'OBSERVER':'AWAITING OUTCOME'
  };
  ...
  function ensureInspector() {
    ...
    if (!box) {
      ...
      (primary || document.body).appendChild(box);
    }
    return box;
  }
  ...
  function visibleWorkspaceTargets() {
    return ['active-analysis','evidence','underwriting-workspace','perspective-summary','conflict-intelligence','independent-risk','investment-case','decision-gate','observer','learning','memory','workstation-inspector'];
  }

  function setWorkspaceVisibility(targetId) {
    visibleWorkspaceTargets().forEach(id => {
      const el = $(id);
      if (!el) return;
      const show = id === targetId;
      el.classList.toggle('workstation-hidden', !show);
  ...
  function inspectorHtml(title, subtitle, body) {
    const box = ensureInspector();
  ...
  function inspectGate() {
    ...
    const cells = [
      ['READINESS',readiness.status || gate.state || 'NO DATA'],
  ...
  function inspectCase() {
    ...
    const c = data.institutional_investment_case || {};
    const r = data.decision_readiness || {};
    const sections = [
      ['STATUS',c.status || r.status || 'NO DATA'],
      ['EVIDENCE',data.evidence?.usable_count == null ? 'NOT EXPOSED' : `${data.evidence.usable_count} USABLE`],
      ['INTERPRETATION',c.interpretation || c.thesis || 'NOT EXPOSED'],
      ['READINESS',r.status || 'NOT EXPOSED']
    ];
    inspectorHtml('Investment Case','ANALYTICAL SYNTHESIS',`<div class="workstation-grid">${sections.map(x => `<div class="workstation-cell"><span>${esc(x[0])}</span><strong>${esc(typeof x[1] === 'object' ? JSON.stringify(x[1]) : x[1])}</strong></div>`).join('')}</div><p>The Investment Case is analytical synthesis. It is not human authorization.</p>`);
  }

  function inspectBoundary(key) {
    const messages = {
    ...
    inspectorHtml(stageMap[key]?.[0] || String(key).toUpperCase(),'BOUNDARY / MATURITY INSPECTOR',`<p>${esc(messages[key] || 'NOT YET OPERATIONAL')}</p>`);
  }
  ...
  function inspectConflicts() {
    ...
    const body = items.length ? items.map((x,i) => {
      const r = x.conflict_record || x;
      const questions = Array.isArray(r.unresolved_questions) ? r.unresolved_questions.join(' • ') : r.unresolved_questions || 'UNRESOLVED QUESTION NOT EXPOSED';
      return `<div class="workstation-cell"><span>CONFLICT ${i + 1} / ${esc(r.conflict_type || x.type || 'CONFLICT')}</span><strong>${esc(r.severity || x.severity || 'UNSPECIFIED')}</strong><p>${esc(questions)}</p></div>`;
    }).join('') : '<p>NO DATA / NO DISAGREEMENT RETURNED. Conflict is never manufactured by the interface.</p>';
    inspectorHtml('Conflict Intelligence','DISAGREEMENT INSPECTOR',body);
  }
  ...
  function inspectPerspective(card) {
    ...
    q('.agent').forEach(x => x.classList.remove('workstation-active'));
    card?.classList.add('workstation-active');
    state.activePerspective = id;
    if (!data) { inspectorHtml(name,'PERSPECTIVE INSPECTOR','<p>NO DATA. No returned output is available for this perspective.</p>'); return; }
    const fields = Object.entries(data).filter(([k]) => !['agent','raw_response'].includes(k)).slice(0,14);
    inspectorHtml(name,'PERSPECTIVE INSPECTOR',`<div class="workstation-grid">${fields.map(([k,v]) => `<div class="workstation-cell"><span>${esc(k.replaceAll('_',' ').toUpperCase())}</span><strong>${esc(typeof v === 'object' ? JSON.stringify(v) : v)}</strong></div>`).join('')}</div>`);
  }

  function inspectContrarian() {
    exposeLatestAnalysis();
    const s = window.latestAnalysis?.skeptic;
    if (!s) { inspectorHtml('Contrarian','THESIS-DESTRUCTION INSPECTOR','<p>NO DATA. No returned contrarian or skeptic output is available.</p>'); return; }
    const fields = Object.entries(s).filter(([k]) => !['raw_response'].includes(k)).slice(0,14);
    inspectorHtml('Contrarian','THESIS-DESTRUCTION INSPECTOR',`<div class="workstation-grid">${fields.map(([k,v]) => `<div class="workstation-cell"><span>${esc(k.replaceAll('_',' ').toUpperCase())}</span><strong>${esc(typeof v === 'object' ? JSON.stringify(v) : v)}</strong></div>`).join('')}</div><p>Contrarian analysis challenges the thesis. It is not an authorization decision.</p>`);
  }
  ...
  function activate(name, detail, meta = {}) {
    ...
    setWorkspaceVisibility(mapped[1] || 'workstation-inspector');
    const target = mapped[1] ? $(mapped[1]) : null;
    setActiveTarget(target || inspector);
    updateContext(key, detail);
    announce(`Active workstation context: ${mapped[0]}`);

    if (key === 'case') inspectCase();
    else if (key === 'gate') inspectGate();
    else if (key === 'human' || key === 'decision record' || key === 'outcome' || key === 'attribution' || key === 'disposition') inspectBoundary(key);
    else if (key === 'risk') inspectRisk();
    else if (key === 'conflict') inspectConflicts();
    else if (key === 'contrarian') inspectContrarian();
  }

  function normalizeNavLabel(item) {
    return text(item).replace(/\s+NOT EXPOSED$/i,'').trim().toLowerCase();
  }
  ...
  function interceptNavigationEvents() {
    document.addEventListener('click', event => {
      ...
      if (nav && !nav.classList.contains('disabled')) {
        const key = navMap[normalizeNavLabel(nav)];
        if (key) {
          event.preventDefault();
          event.stopImmediatePropagation();
          activate(key, `Workspace: ${text(nav)}`);
          return;
        }
      }
      const actionMap = {
        'open-analysis':'opportunity',
        'open-simulation':'risk',
        'open-learning':'learning',
        'view-conflicts':'conflict',
        'view-gate-details':'gate'
      };
      const action = event.target.closest?.('button');
      if (action && actionMap[action.id]) {
        event.preventDefault();
        event.stopImmediatePropagation();
        const key = actionMap[action.id];
        activate(key, `Workspace action: ${text(action)}`);
        if (key === 'risk') inspectRisk();
        if (key === 'conflict') inspectConflicts();
        if (key === 'gate') inspectGate();
      }
    }, true);
    ...
    document.addEventListener('keydown', event => {
      ...
      if (input?.id !== 'global-search') return;
      event.stopImmediatePropagation();
      event.preventDefault();
    }, true);
  }
  ...
  function wireInspectors() {
    ...
    if (risk && !risk.dataset.workstationBound) {
      ...
      risk.addEventListener('click', e => {
        const card = e.target.closest('.risk-card'); if (!card) return;
        q('.risk-card').forEach(x => x.classList.remove('workstation-active'));
        card.classList.add('workstation-active');
        const label = text(card.querySelector('span'));
        state.activeRiskScenario = label;
        exposeLatestAnalysis();
        const scenarios = window.latestAnalysis?.simulation?.scenarios || [];
        const raw = Array.isArray(scenarios) ? scenarios.find(x => String(x.scenario || '').toLowerCase() === label.toLowerCase()) : null;
        inspectorHtml(label,'RISK SCENARIO INSPECTOR',raw ? `<div class="workstation-grid">${Object.entries(raw).map(([k,v]) => `<div class="workstation-cell"><span>${esc(k.replaceAll('_',' ').toUpperCase())}</span><strong>${esc(typeof v === 'object' ? JSON.stringify(v) : v)}</strong></div>`).join('')}</div>` : '<p>NO DATA. This scenario has no returned backend record.</p>');
      });
    }
    const conflicts = $('conflicts');
    if (conflicts && !conflicts.dataset.workstationBound) {
      conflicts.dataset.workstationBound = '1';
      conflicts.addEventListener('click', e => { const item = e.target.closest('.conflict-item'); if (item) { q('.conflict-item').forEach(x => x.classList.remove('workstation-active')); item.classList.add('workstation-active'); state.activeConflict = text(item); inspectConflicts(); } });
    }
  }
  ...
  function wireRecent() {
    ...
    root.addEventListener('click', event => {
      ...
      const question = text(item.querySelector('strong')) || 'ACTIVE ANALYSIS';
  ...
  function wireSearch() {
    ...
    if (!input || input.dataset.workstationBound) return;
    input.dataset.workstationBound = '1';
    let box = $('workstation-search-results');
    if (!box) {
      box = document.createElement('div');
      box.id = 'workstation-search-results';
      box.className = 'workstation-search-results';
      box.hidden = true;
    ...
    input.addEventListener('input', () => {
      ...
      let html = '', n = 0;
      groups.forEach(([label,selector]) => {
        const hits = q(selector).filter(el => text(el).toLowerCase().includes(term));
        if (!hits.length) return;
        html += `<div class="workstation-search-group">${label}</div>`;
        hits.slice(0,8).forEach(el => { const id = `r${n++}`; el.dataset.workstationResult = id; html += `<button type="button" class="workstation-search-result" data-result="${id}">${esc(text(el).slice(0,100) || 'Untitled')}<small>${label}</small></button>`; });
      });
      box.innerHTML = html || '<div class="workstation-search-group">NO MATCHING RETURNED WORKSTATION DATA</div>';
      box.hidden = false;
      q('.workstation-search-result',box).forEach(btn => btn.addEventListener('click', () => {
        const el = document.querySelector(`[data-workstation-result="${CSS.escape(btn.dataset.result)}"]`);
        if (el) {
          const key = el.classList.contains('stage') ? text(el.querySelector('.stage-name')) : el.closest('.agent') ? 'perspectives' : el.closest('.conflict-item') ? 'conflict' : el.closest('.activity-item') ? 'observer' : 'opportunity';
          activate(key,text(el));
        }
        box.hidden = true;
        input.blur();
      }));
  ...
  function correctMaturityLabels() {
    q('.future-row,.pipeline .stage').forEach(row => {
      const label = text(row).toUpperCase();
      Object.entries(maturity).forEach(([name,status]) => {
        if (!label.includes(name)) return;
        const badge = row.querySelector('.future-row b,.stage-status');
        if (badge) badge.textContent = status;
      });
    });
    const memoryPanel = document.querySelector('.memory-panel .state-badge');
    if (memoryPanel) { memoryPanel.textContent = 'DEVELOPING'; memoryPanel.className = 'state-badge developing'; }
  }

  function addStyles() {
    if ($('workstation-styles')) return;
    const s = document.createElement('style');
    s.id = 'workstation-styles';
    s.textContent = `
  ...
  function overrideLegacyScroll() {
    window.scrollToId = id => {
      const target = Object.entries(stageMap).find(([,m]) => m[1] === id);
      if (target) activate(target[0], `Workspace: ${target[0].toUpperCase()}`);
      else activate('opportunity',`Workspace target: ${id}`);
    };
    document.documentElement.style.scrollBehavior = 'auto';
  }

  function observeDynamicContent() {
    ['pipeline-stages','recent-analyses','perspectives','risk-grid','conflicts'].map($).filter(Boolean).forEach(root => new MutationObserver(() => {
      exposeLatestAnalysis();
      syncCase();
      correctMaturityLabels();
      wireInspectors();
